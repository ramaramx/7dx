...
const ALL_TARGETS = ['vb-target-01', 'vb-target-02', 'vb-target-03', 'vb-target-04', 'vb-target-05', 'vb-target-06']
      ...
      if (activeVideo) {
        ...
        setTimeout(() => {
          if (activeMarker && activeMarker.star) {
            ...
            activeMarker.star.addEventListener('animationcomplete__scaleout', () => {
  ...
  collectButton.addEventListener('click', () => {
    if (activeItemModel && collectButtonVisible) {
      ...
      if (activeItemModel) {
        ...
        activeItemModel.addEventListener('animationcomplete__scale', () => {
      ...
      if (!foundTargets[activeItemModel.targetName]) {
        ...
        console.log('Target handler: First time finding target:', activeItemModel.targetName)
...
let foundTargets = ALL_TARGETS.reduce((acc, target) => ({ ...acc, [target]: false }), {})
...
const updateIconOpacity = (targetName, isPreviouslyFound = false) => {
  console.log('Updating icon for target:', targetName, 'isPreviouslyFound:', isPreviouslyFound)
...
AFRAME.registerComponent('panic-mode', {
  init() {
    ...
    const targets = ['vb-target-01', 'vb-target-02', 'vb-target-03', 'vb-target-04', 'vb-target-05', 'vb-target-06']
    const found = []
...
AFRAME.registerComponent('target-handler', {
  init() {
    ...
    const frameGeometry = {
      ...
      vertices: [
        // Outer rectangle
        -frameWidth/2, -frameHeight/2, 0,  // Bottom left
        frameWidth/2, -frameHeight/2, 0,   // Bottom right
        frameWidth/2, frameHeight/2, 0,    // Top right
        -frameWidth/2, frameHeight/2, 0,   // Top left
        // Inner rectangle (matches video size exactly)
        -videoWidth/2, -videoHeight/2, 0,  // Bottom left
        videoWidth/2, -videoHeight/2, 0,   // Bottom right
        videoWidth/2, videoHeight/2, 0,    // Top right
        -videoWidth/2, videoHeight/2, 0    // Top left
      ],
      faces: [
        // Outer border faces
        0, 1, 5,  0, 5, 4,  // Bottom
    ...
    videoScreen.addEventListener('animationcomplete__position', () => {
      ...
      if (this.star) {
        ...
        // Hide star after animation
        this.star.addEventListener('animationcomplete__scaleout', () => {
    ...
    this.itemModel.setAttribute('gltf-model', `#item-${targetNum.toString().padStart(2, '0')}`)
